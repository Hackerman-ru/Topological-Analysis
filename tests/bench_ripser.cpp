...
// std::vector<coefficient_t> multiplicative_inverse_vector(
...
// template <typename Entry>
// struct smaller_diameter_or_greater_index {
//     bool operator()(const Entry& a, const Entry& b) {
//         return (get_diameter(a) < get_diameter(b)) ||
//                ((get_diameter(a) == get_diameter(b)) &&
//                 (get_index(a) > get_index(b)));
...
//         : distances(mat.size() * (mat.size() - 1) / 2),
//           rows(mat.size()) {
//         init_rows();
...
//     template <typename DistanceMatrix>
//     sparse_distance_matrix(const DistanceMatrix& mat, const value_t
//     threshold)
//         : neighbors(mat.size()),
//           num_edges(0) {
...
//             if (get_index(neighbor) == j)
//                 return get_diameter(neighbor);
...
//             std::plus<value_t>(), [](value_t u, value_t v) {
//                 return (u - v) * (u - v);
//             }));
//     }
...
//             return hash<index_t>()(::get_index(e));
//         }
//     };
...
//     typedef hash_map<entry_t, size_t, entry_hash, equal_index>
//     entry_hash_map;

//    public:
//     std::vector<std::pair<value_t, value_t>> pairs;
...
//         return get_max(n, k - 1, [&](index_t w) -> bool {
...
//         vertices.clear();
//         get_simplex_vertices(index, dim, dist.size(),
...
//                                     const index_t _dim, const ripser&
//                                     _parent)
...
//                 (k & 1 ? -1 + modulus : 1) * get_coefficient(simplex) %
//                 modulus;
...
//             simplex_coboundary_enumerator cofacets(diameter_entry_t(simplex,
//             1),
//                                                    dim, *this);
...
//             vertices_of_edge.rbegin()); index_t u =
//             dset.find(vertices_of_edge[0]),
...
//             pivot = column.top();
//             column.pop();
//             if (column.empty() || get_index(column.top()) !=
//             get_index(pivot))
//                 return pivot;
//             column.pop();
//         }
...
//         Column& working_coboundary) {
//         diameter_entry_t
//         column_to_add(columns_to_reduce[index_column_to_add],
//                                        factor);
//         add_simplex_coboundary<BoundaryEnumerator>(
//             column_to_add, dim, working_reduction_column,
//             working_coboundary);
...
//     template <typename BoundaryEnumerator, typename Sorter,
//               bool cohomology = true>
//     void compute_pairs(const std::vector<diameter_index_t>&
//     columns_to_reduce,
//                        entry_hash_map& pivot_column_index, const index_t dim)
//                        {
// #ifdef PRINT_PERSISTENCE_PAIRS
//         if (!cohomology)
//             std::cout << "persistence intervals in dim " << dim - 1 << ":"
//                       << std::endl;
// #endif
...
//                 init_coboundary_and_get_pivot<BoundaryEnumerator>(
...
//                             std::cerr << clear_line << std::flush;
// #endif
//                             pairs.emplace_back(death, diameter);
//                         }
//                     }
//                     break;
//                 }
...
//             for (auto it = pivot_column_index.begin();
//                  it != pivot_column_index.end(); ++it) {
//                 auto pair = *it;

//                 index_t pivot_row_index = pair.first;
//                 boundary_columns_to_reduce.push_back(
//                     std::make_pair(compute_diameter(pivot_row_index, dim +
//                     1),
//                                    pivot_row_index));

//                 // TODO: essential indices missing (required for
//                 thresholding)
//             }

//             std::sort(boundary_columns_to_reduce.rbegin(),
//                       boundary_columns_to_reduce.rend(),
//                       greater_diameter_or_smaller_index<diameter_index_t>());

//             entry_hash_map boundary_pivot_column_index;
//             boundary_pivot_column_index.reserve(
//                 boundary_columns_to_reduce.size());

//             compute_pairs<simplex_boundary_enumerator,
//                           smaller_diameter_or_greater_index<diameter_entry_t>,
//                           false>(boundary_columns_to_reduce,
//                                  boundary_pivot_column_index, dim + 1);
...
//         return (j >= k && (all_cofacets || binomial_coeff(j, k) >
//         idx_below));
//     }
...
//         index_t cofacet_index =
//             idx_above + binomial_coeff(get_index(neighbor), k + 1) +
//             idx_below;
//         coefficient_t cofacet_coefficient =
//             (k & 1 ? modulus - 1 : 1) * get_coefficient(simplex) % modulus;
//         return diameter_entry_t(cofacet_diameter, cofacet_index,
...
// sparse_distance_matrix read_sparse_distance_matrix(std::istream&
// input_stream) {
//     std::vector<std::vector<index_diameter_t>> neighbors;
//     index_t num_edges = 0;
...
// compressed_lower_distance_matrix read_lower_distance_matrix(
...
// compressed_lower_distance_matrix read_upper_distance_matrix(
//     std::istream& input_stream) {
//     std::vector<value_t> distances;
//     value_t value;
//     while (input_stream >> value) {
...
// compressed_lower_distance_matrix read_distance_matrix(
//     std::istream& input_stream) {
//     std::vector<value_t> distances;
...
//     compressed_lower_distance_matrix dist = read_file(file_stream,
//     POINT_CLOUD);

//     value_t min = std::numeric_limits<value_t>::infinity(),
//             max = -std::numeric_limits<value_t>::infinity(), max_finite =
//             max;
//     int num_edges = 0;
