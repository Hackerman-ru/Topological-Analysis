...
#define TEST(name) \
    TEMPLATE_TEST_CASE_SIG(name, "", ((size_t N), N), 100, 250, 500)
...
TEST("Benchmark GetFacetsPos") {
    ...
    BENCHMARK("FlatTree::GetFacetsPos (triangles)") {
        size_t total = 0;
...
TEST("Benchmark GetCofacetsPos") {
    ...
    BENCHMARK("FlatTree::GetCofacetsPos (edges)") {
        size_t total = 0;
