...
EigenMatrix CutMatrix(const EigenMatrix& matrix, const FilteredComplex& complex,
    ...
    const auto sizes = order == 1 ? std::make_pair(1, 2) : std::make_pair(2, 3);
    std::vector<size_t> rows = get_indices(sizes.first);
    ...
    for (size_t j = 0; j < cols.size(); ++j) {
        for (EigenMatrix::InnerIterator it(matrix, cols[j]); it; ++it) {
            auto row_it = std::lower_bound(rows.begin(), rows.end(), it.row());
            if (row_it != rows.end() &&
